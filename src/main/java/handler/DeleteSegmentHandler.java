...
public class DeleteSegmentHandler implements RequestHandler<DeleteSegmentRequest,DeleteSegmentResponse>{
	...
	public DeleteSegmentResponse handleRequest(DeleteSegmentRequest req, Context context) {
		...
		Segment s = new Segment(req.id);
		try {
			if (dao.deleteSegment(s)) {
				response = new DeleteSegmentResponse(req.id, 200);
			} else {
				response = new DeleteSegmentResponse(req.id, 422, "Unable to delete Playlist.");
			}
		} catch (Exception e) {
			response = new DeleteSegmentResponse(req.id, 403, "Unable to delete Playlist: " + req.id+ "(" + e.getMessage() + ")");
		}
