 ...
 */
public class listVideoSegmentHandler implements RequestHandler<Object,AllConstantsResponse> {
	...
	List<Segment> systemConstants() throws Exception {
		...
		ListObjectsV2Request listObjectsRequest = new ListObjectsV2Request()
				  .withBucketName("cs3733wpi")    // top-level bucket
		...
		for (S3ObjectSummary os: objects) {
	      ...
	      S3Object obj = s3.getObject("cs3733wpi", name);
	    	...
	    	try (S3ObjectInputStream constantStream = obj.getObjectContent()) {
				...
				sysConstants.add(new Segment(name.substring(postSlash+1), Double.valueOf(val), true));
			} catch (Exception e) {
